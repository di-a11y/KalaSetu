...
const Navbar = () => {
  ...
  // ✅ FIX: all from ONE useContext call — no more double declaration
  const {
    search,
    setSearch,
    showSearch,
  ...
  return (
    <>
      {/*
        SearchBar lives OUTSIDE the <header> so it can overlay
        the entire page with its backdrop. It self-hides when
        showSearch is false (returns null).
      */}
      <SearchBar />

      <header className="w-full font-medium bg-[#B7A167] relative z-40">
        ...
        <div className="flex items-center justify-between px-8 py-5">
          ...
          <div className="flex items-center gap-6">
            ...
            {/*
              ✅ Search icon — clicking sets showSearch=true which
              renders the full SearchBar overlay (like Adidas)
            */}
            <button
              onClick={() => { setShowSearch(true); setSearch(''); }}
          ...
          <div className={`fixed top-0 right-0 bottom-0 z-50 overflow-hidden bg-white transition-all duration-300 ${visible ? 'w-full' : 'w-0'}`}>
            <div className="flex flex-col text-gray-600">
              <div onClick={() => setVisible(false)} className="flex items-center gap-4 p-3 cursor-pointer">
                <img className="h-4 rotate-180" src={dropdown_icon} alt="back" />
                <p>Back</p>
              </div>
              <NavLink onClick={() => setVisible(false)} className="py-2 pl-6 border" to="/About">About</NavLink>
              <NavLink onClick={() => setVisible(false)} className="py-2 pl-6 border" to="/Handicrafts">Handicrafts</NavLink>
              <NavLink onClick={() => setVisible(false)} className="py-2 pl-6 border" to="/Clothing">Clothing</NavLink>
